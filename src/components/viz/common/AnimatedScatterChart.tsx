...
interface ScatterChartProps {
  data: AnimatedScatterChartDataPoint[];
  xLabel?: string;
  yLabel?: string;
  chartTitle?: string;
}

const AnimatedScatterChart: React.FC<ScatterChartProps> = ({
  data,
  xLabel = 'x',
  yLabel = 'y',
  chartTitle = ''
  ...
  const drawChart = () => {
    ...
    const buildDetails = (details: object) => {
      if (!details) return '';
      return Object.entries(details).map(([key, value]) => {
        return `${_.upperFirst(key)}: ${value}`;
      }).join('<br/>');
    }

    svg.selectAll<SVGCircleElement, AnimatedScatterChartDataPoint>('.hit-area')
      .on('mouseover', function (event, d) {
        tooltip.transition()
          .duration(200)
          .style('opacity', 1);

        tooltip.html(`${xLabel}: ${d.x}<br/>${yLabel}: ${d.y.toFixed(2)}<br/>${buildDetails(d.details)}`)
    ...
    const zoom = d3.zoom<SVGSVGElement, unknown>()
      .on("zoom", (event) => {
        ...
        yScaleRef.current = event.transform.rescaleY(originalYScale);
        ...
        svg.selectAll<SVGCircleElement, AnimatedScatterChartDataPoint>('.dot')
          .attr('cx', d => xScaleRef.current ? xScaleRef.current(d.x) : 0)
          .attr('cy', d => yScaleRef.current ? yScaleRef.current(d.y) : 0);
